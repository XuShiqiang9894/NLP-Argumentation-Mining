...
class ArgMiningBiLSTM(ArgMiningModel):
    ...
    def train_and_predict(self):
        ...
        word_to_index = dict()
        i = 1
        with open(embedding_file_en, 'r', encoding='utf-8') as f:
            lines = f.readlines()
            for line in lines:
                values = line.split()
                word = values[0]
                word_to_index[word] = i
                coeds = np.array(values[1:], dtype='float32')
                embedding_matrix[i] = coeds
                i += 1
        ...
        X_dev_padded = X_dev_index + [[400001] for _ in
        ...
        y_train = sequence.pad_sequences(
            np.asarray([to_categorical(y_label, number_of_classes + 1) for y_label in y_train_padded]),
            maxlen=max_sentence_length)
        y_dev = sequence.pad_sequences(
            np.asarray([to_categorical(y_label, number_of_classes + 1) for y_label in y_dev_padded]),
        ...
        def build_model(embedding_matrix, max_sentence_length, number_of_classes):
            ...
            model.add(
                Bidirectional(LSTM(hidden_units, input_shape=(batch_size, max_sentence_length), return_sequences=True)))
            model.add(
                Bidirectional(LSTM(hidden_units, input_shape=(batch_size, max_sentence_length), return_sequences=True)))
            model.add(Dropout(dropout))
